...
@Component({
  ...
  styleUrls: ['./os-finalizadas.component.css']
})
export class OsFinalizadasComponent implements AfterViewInit, OnInit {
...
'comentarios', 'deletar'];
  ...
  pesquisar(pagina = 0, limite = 5, nome = ''){
    ...
    .subscribe(resultado =>{ 
      ...
      this.dados = this.dataSource.data.filter((dados: Os) => dados.status === 'FINALIZADA' || dados.status === 'CANCELADA');
